...
/**
 * @author Desconhecido Classe para escrever um valor por extenso
 */
...
public class NumberUtil {
  ...
  private static final String QUALIFICADORES[][] = { {"centavo", "centavos"}, {"", ""},
   ...
   */
  public NumberUtil(BigDecimal dec) {
    this();
    setNumber(dec);
  }
  ...
  public String toString() {
    ...
    for (ct = nro.size() - 1; ct > 0; ct--) {
      ...
      if (sb.length() > 0 && !ehGrupoZero(ct)) {
        sb.append(" e ");
      }
      sb.append(numToString(((Integer) nro.get(ct)).intValue(), ct));
    }
    if (sb.length() > 0) {
      if (ehUnicoGrupo()) {
        sb.append(" de ");
    ...
    if (((Integer) nro.get(0)).intValue() != 0) {
      sb.append(numToString(((Integer) nro.get(0)).intValue(), 0));
    }
    return sb.toString();
  }
